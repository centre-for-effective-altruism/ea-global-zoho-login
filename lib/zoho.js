...
export const handleRedirectCallback = async ({ code }) => {
  ...
  const tokenRes = await ZCRMRestClient.generateAuthTokens(null, code)
  console.log(tokenRes)
}

const authCheck = async () => {
  const res = await ZCRMRestClient.API.MODULES.get({
    module: 'Contacts',
    params: {
      page: 0,
      per_page: 5
  ...
  if (res.statusCode > 400 || body.status === 'error') {
    throw new Error(body.message || body.toString())
  }
  return body.data
}
...
export const createToken = async (user_identifier = ZOHO_USER_IDENTIFIER, grant_token = ZOHO_GRANT_TOKEN) => {
  console.log(`Getting auth tokens from Zoho for user ${user_identifier}`)
  await ZCRMRestClient.generateAuthTokens(user_identifier, grant_token)
  await authCheck()
  return ZCRMRestClient
}
